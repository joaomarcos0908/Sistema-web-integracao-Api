...
async function buscarCotacoes() {
  try {
    ...
    console.warn('Cotação indisponível, usando fallback:', err);
    return { dolar: '5.10', euro: '5.55' };
  }
}
 
async function fetchCotacoes() {
  const cotacoes = await buscarCotacoes();
  taxaDolar = parseFloat(cotacoes.dolar);
  taxaEuro  = parseFloat(cotacoes.euro);
  renderCurrencyBar();
  atualizarPrecosConvertidos();
}
...
function renderCurrencyBar() {
  ...
  if (!bar) return;
  bar.innerHTML = `
    <span>💱 Cotação ao vivo</span>
    <span class="sep">|</span>
    <span class="rate-item">🇺🇸 Dólar: <strong>R$ ${taxaDolar.toFixed(2)}</strong></span>
    <span class="sep">|</span>
    <span class="rate-item">🇪🇺 Euro: <strong>R$ ${taxaEuro.toFixed(2)}</strong></span>
    <span class="sep">|</span>
    <span style="font-size:0.65rem;opacity:0.6">Fonte: AwesomeAPI · ${new Date().toLocaleTimeString('pt-BR')}</span>
  `;
}
 
// ✅ ERRO 1 CORRIGIDO — crase adicionada
...
function atualizarPrecosConvertidos() {
  document.querySelectorAll('[data-preco-brl]').forEach(el => {
    const brl = parseFloat(el.dataset.precoBrl);
    el.textContent = converterPreco(brl);
  });
}
...
function renderObras(lista) {
  ...
  // ✅ ERRO 2 CORRIGIDO — crases adicionadas nas template strings
...
// ✅ ERRO 3 CORRIGIDO — apenas um DOMContentLoaded
